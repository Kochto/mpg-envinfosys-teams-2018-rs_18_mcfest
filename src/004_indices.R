...
pca <- RStoolbox::rasterPCA(indices, spca = TRUE)
...
eigvalue.pca <- factoextra::get_eigenvalue(pcamin$model)
factoextra::fviz_eig(pcamin$model, addlabels = TRUE,ylim=c(0,50))
var.pca <- factoextra::get_pca_var(pcamin$model)
corrplot::corrplot(var.pca$contrib,is.corr = FALSE)
factoextra::fviz_pca_var(pcamin$model, col.var = "cos2",
                         gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                         alpha.var = "cos2",# add transparency according to cos2-values
                         repel = TRUE # Avoid text overlapping
)
