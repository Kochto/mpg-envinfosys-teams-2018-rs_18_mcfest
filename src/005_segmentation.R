...
laubFT <- uavRst::treepos_FT(chm=laub ,minTreeAlt = 2, maxCrownArea = 150)
...
laubRL <- uavRst::treepos_RL(chm=laub, movingWin = 3, minTreeAlt = 2)
...
crlaubITC <- uavRst::chmseg_ITC(chm = laub, EPSG = 25832, minTreeAlt = 2, maxCrownArea = 150)
...
nadellaubFT <- uavRst::treepos_FT(chm=nadellaub ,minTreeAlt = 2, maxCrownArea = 150)

#Parameter to play around with
path_run <- envrmt$path_run
path_tmp <- envrmt$path_data_tmp
nadellaubGWS <- uavRst::treepos_GWS(chm=nadellaub, minTreeAlt = 2, minCrownArea = 3, maxCrownArea = 150, 
                               join = 1, thresh = 0.35, split = TRUE, 
                               cores = 2, giLinks = giLinks)
...
crnadellaubITC <- uavRst::chmseg_ITC(chm = nadellaub, EPSG = 25832, minTreeAlt = 2, maxCrownArea = 150)

#Parameter to play around with
crnadellaubGWS <- uavRst::chmseg_GWS(chm = nadellaub , treepos = nadellaubGWS, minTreeAlt = 2,
                                neighbour = 0,
                                thVarFeature = 1.,
                                thVarSpatial = 1.,
                                thSimilarity = 0.00001,
...
writeOGR(crnadellaubFT, paste0(envrmt$path_data_lidar_segtest_nadel_laub_test, "crnadellaubFT.shp"), "crnadellaubFT", driver="ESRI Shapefile")
...
nadelFT <- uavRst::treepos_FT(chm=nadel ,minTreeAlt = 2, maxCrownArea = 150)
...
crnadelITC <- uavRst::chmseg_ITC(chm = nadel, EPSG = 25832, minTreeAlt = 2, maxCrownArea = 150)
...
writeOGR(crnadelITC, paste0(envrmt$path_data_lidar_segtest_nadeltest, "crnadelITC.shp"), "crnadelITC", driver="ESRI Shapefile")
